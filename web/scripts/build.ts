...
const WEB_ROOT = path.resolve(path.dirname(process.argv[1]), '..');
const DIST_ROOT = path.join(WEB_ROOT, '../dist');

build({
    entryPoints: [
        path.join(WEB_ROOT, 'src/index.ts'),
        path.join(WEB_ROOT, 'src/worker.ts'),
    ],
    bundle: true,
    sourcemap: true,
    minify: false,
...
}, true, path.join(DIST_ROOT, 'web-meta.json'));
...
async function build(opts: BuildOptions, startServer: boolean, metaFileName: string) {
    ...
    if (startServer && mode === 's') {
        ...
        console.log(`    http://${result.hosts[0]}:${result.port}/`);
    } else if (mode !== '') {
        await ctx.watch();
    } else {
