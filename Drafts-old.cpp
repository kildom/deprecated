...
struct FunctionRuntimeData
{                        // Only for async and generator functions, normal functions will use common stack.
...
ObjectData* extendHeapObjectData(MyesContext* ctx, ObjectHead* head, size_t propertiesCount)
{
    ...
    ObjectData* newData =
        (ObjectData*)reallocHeap(ctx, data, propertiesEnd + 2 * sizeof(Value) * propertiesCount + sizeof(Value));
...
static inline bool stringKeyEq(Value a, Value b)
{
    ...
    uintptr_t aHash =
        aData
            ->hash;  // TODO: consider moving 'hash' to head (instead of 'length'). It will be probably more ofter used.
    ...
    if (emptySlotKeyPtr == NULL) {
        ...
        if (emptySlotKeyPtr + 2 >= endPtr) {
            ...
            if (emptyCount > 8 && emptyCount > usedCount) {
                emptySlotKeyPtr = compactObject(
                    object);  // Remove empty RAM-only slots and move the reset, don't reallocate, return pointer to END, it will always success
            } else if (heapAppending(object,
                           2 * sizeof(Value))) {  // Not ready for compacting, so resize it and calculate new pointer
