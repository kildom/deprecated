...
let getStackPointerFuncIndex: number;
...
export function processModule2(moduleBinary: Uint8Array, currentMemory: Uint8Array, currentStackPointer: number, sizeOptimize: boolean) {
    ...
    while (offset < moduleBinary.length) {
        let id = bin[offset++];
        let size = leb128();
...
export function processModule(moduleBinary: Uint8Array, currentMemory: Uint8Array, currentStackPointer: number, sizeOptimize: boolean) {
    ...
    while (offset < bin.length) {
        ...
        console.log('Section', outSize(0), id);
        switch (id) {
            ...
            case SectionType.functionSection:
                console.log(out.length, out[0]!.length, out[1]!.length, out[2]!.length, outSize(0));
            ...
            case SectionType.exportSection:
                parseExportSection(sectionStart, sectionEnd, false);
                break;
            ...
            case SectionType.codeSection:
                parseCodeSection(sectionStart, sectionEnd, false);
                break;
...
export function parseModule(moduleBinary: Uint8Array) {

    offset = 0;
    bin = moduleBinary;
    memory = undefined as any;
    stackPointer = undefined as any;
    out = [];
    setSpTypeIndex = -1;
    getSpTypeIndex = -1;
    getSpFuncIndex = -1;
    ...
    while (offset < bin.length) {
        ...
        let size = leb128();
        let sectionEnd = offset + size;
        console.log(`Section ${id} at ${sectionStart} of size ${sectionEnd - sectionStart}`);
        switch (id) {
            case SectionType.importSection:
                parseImportSection(sectionStart, sectionEnd);
                break;
            ...
            case SectionType.globalSection:
                ...
                break;
            case SectionType.exportSection:
                parseExportSection(sectionStart, sectionEnd, true);
                break;
            case SectionType.codeSection:
                parseCodeSection(sectionStart, sectionEnd, true);
                break;
            default:
                offset += size;
                break;
...
function parseImportSection(sectionStart: number, sectionEnd: number) {
    ...
    console.log('import', sectionStart, sectionEnd);
...
function parseFunctionSection(sectionStart: number, sectionEnd: number) {
    ...
    console.log('function', sectionStart, sectionEnd, new Uint8Array(bin.subarray(sectionStart, sectionStart + 1)));
...
function parseExportSection(sectionStart: number, sectionEnd: number, parseOnly: boolean) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (name === 'getStackPointer') {
            getStackPointerFuncIndex = index;
    ...
    actual_count++;
    if (!parseOnly) assert.equal(actual_count, count);
    replace(sectionSizeOutBookmark, outSize(sectionSizeOutBookmark + 1));
    assert.equal(offset, sectionEnd);
    assert(stackPointerIndex >= 0 || getStackPointerFuncIndex >= 0);
}
...
function parseMemorySection(sectionStart: number, sectionEnd: number) {
    ...
    let expectedBlocks = memory
        ? Math.max(min, Math.ceil(memory.length / BLOCK_SIZE)) + MEM_PRE_ALLOC / BLOCK_SIZE
        : min;
    ...
    console.log(reshapeModuleParams.memLimitsOffset, reshapeModuleParams.memLimitsOffset.toString(16));
}

function parseCodeSection(sectionStart: number, sectionEnd: number, parseOnly: boolean): void {
    ...
    for (let i = 0; i < count; i++) {
        let size = leb128();
        let index = i + funcIndexStart;
        console.log(index, getStackPointerFuncIndex);
        if (index === getStackPointerFuncIndex) {
            let locals = leb128();
            assert.equal(locals, 0);
            let instr = bin[offset++];
            assert.equal(instr, 0x23);
            stackPointerIndex = leb128();
            instr = bin[offset++];
            assert.equal(instr, 0x0B);
        } else {
            offset += size;
        }
    }
...
export function appendCustomSection(moduleBin: Uint8Array): Uint8Array {
    ...
    console.log(stackPointerIndex - globalIndexStart);
    let offsets = globalsOffsets[stackPointerIndex - globalIndexStart];
    reshapeModuleParams.stackPointerValueBegin = offsets.start;
    reshapeModuleParams.stackPointerValueEnd = offsets.end;

    output(SectionType.customSection);
    output(1 + CUSTOM_SECTION_NAME.length + 6 * 4);
    output(CUSTOM_SECTION_NAME);

    console.log(reshapeModuleParams);

    let struct = new DataView(new ArrayBuffer(6 * 4));
    struct.setUint32(4 * 0, INTERFACE_VERSION, true);
    struct.setUint32(4 * 1, reshapeModuleParams.stackPointerValueBegin, true);
    struct.setUint32(4 * 2, reshapeModuleParams.stackPointerValueEnd, true);
...
function updateStackPointer() {
    ...
    out = [outputBinary];
    let bin = outputBinary.subarray(reshapeModuleParams.stackPointerValueBegin, reshapeModuleParams.stackPointerValueEnd);
    let sp = stackPointer;
...
const sectionParsers: ((section: Section) => void)[] = [];
sectionParsers[SectionType.typeSection] = parseTypeSection;
sectionParsers[SectionType.importSection] = parseImportSection;
sectionParsers[SectionType.functionSection] = parseFunctionSection;
sectionParsers[SectionType.tableSection] = parseTableSection;
sectionParsers[SectionType.memorySection] = parseMemorySection;
sectionParsers[SectionType.globalSection] = parseGlobalSection;
sectionParsers[SectionType.exportSection] = parseExportSection;
sectionParsers[SectionType.startSection] = parseStartSection;
sectionParsers[SectionType.elementSection] = parseElementSection;
sectionParsers[SectionType.codeSection] = parseCodeSection;
sectionParsers[SectionType.dataSection] = parseDataSection;
sectionParsers[SectionType.dataCountSection] = parseDataCountSection;

let currentSection: Section;

function parseSection(section: Section): void {
    currentSection = section;
    bin = section.input;
    offset = 0;
    out = section.output;
    sectionParsers[section.id](section);
}
