...
function parseMap(file) {
	...
	memConf.replace(/(\S+)[\t ]+(0x0{0,8}[0-9A-F]{8})[\t ]+(0x0{0,8}[0-9A-F]{8}).*\r?\n/img, (str, name, addrStr, sizeStr) => {
	...
	memMap.replace(/\n([\t ]*)([^\s]+)\r?\n?[\t ]+(0x0{0,8}[0-9A-F]{8})\s+(0x[0-9A-F]+)([\t ]+[^\r\n]+)?/img, (str, ind, sec, addrStr, sizeStr, file) => {
		let addr = Number.parseInt(addrStr);
		let size = Number.parseInt(sizeStr);
		if (ind == '') {
			let m;
			if (file && (m = file.match(/\s*load address[\t ]+(0x0{0,8}[0-9A-F]{8})/im))) {
...
function readFilterFunction(file) {
	try {
		...
		die('Cannot read filter script file: ' + ex.message ?? ex.toString());
	}
}
...
function main() {
	...
	if (filterFile) {
		filterFunction = readFilterFunction(filterFile);
	}
