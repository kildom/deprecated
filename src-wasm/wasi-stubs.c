...
WASM_IMPORT(sandbox, getTime) uint64_t getTime(uint32_t realTime);
WASM_IMPORT(sandbox, getRandom) void getRandom(uint8_t *buf, uint32_t size);
WASM_IMPORT(sandbox, log) void logMessage(const void* str, uint32_t len);
WASM_IMPORT(sandbox, exit) _Noreturn void wasmExit(uint32_t code);
